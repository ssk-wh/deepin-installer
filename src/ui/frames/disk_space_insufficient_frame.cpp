...
QString GetCommentLabel() {
  ...
  const int fullDiskMin = GetSettingsInt(kPartitionFullDiskMiniSpace);
  if (minimum <= 0) {
      minimum = qMin(GetSettingsInt(kPartitionRootMiniSpace)
                   , GetSettingsInt(kPartitionFullDiskMiniSpace));
  }
  QString tips = ::QObject::tr("You need at least %1 GB of disk space to install the system."
                               " If you want to use the full-disk installation, a minimum of %2 GB is required.")
                .arg(minimum)
                .arg(fullDiskMin);

  if (GetSettingsBool(kPartitionDoAutoPart)) {//如果是自动分区则检测，全盘分区最小磁盘容量要求
      minimum = GetSettingsInt(kPartitionFullDiskMiniSpace);
      tips = ::QObject::tr("The free disk space is less than %1 GB, so you should configure partitions"
                           " manually. To use the full-disk installation, a minimum of %2 GB is required.")
                      .arg(minimum)
                      .arg(fullDiskMin);
  }
